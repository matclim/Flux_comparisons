...
```bash
build/flux_compare A.root B.root
build/flux_compare @listA.txt @listB.txt --label-a nominal --label-b variant
build/flux_compare A.root --closure          # run this first
...
```bash
build/flux_compare A.root --closure-scan 20 --nperm 200 --max-events-per-file 50000
```
...
applied to muons only; run `flux_weight_audit --pdg 14` first, since the
...
`--max-events` takes a **prefix**, not a random subset. An earlier version drew
a hashed pseudo-random subset; comparing one file against itself under that
option produced two different samples, so it was removed. A prefix cannot
desynchronise: two loads of the same list necessarily see the same entries.
The cost is that `Range` and implicit MT are incompatible in RDataFrame, so a
limited read is single-threaded; the run log says so.

Truncation is unbiased provided the production does not order events by
anything physical, but it does cut `N_eff`. Prefer `--max-files` when you want
less data and still want threading.
...
```bash
build/flux_compare @listA.txt @listB.txt --max-events-per-file 2000
build/flux_compare @listA.txt B.root --max-events-a 500000   # balance the two
```
...
```
include/fluxval/Core.h     ROOT-free numerics: EDF statistics, permutation
                            null, block bootstrap, quantile binning
include/fluxval/Config.h   variables, regions, reported quantities
include/fluxval/Load.h     RDataFrame loader (plane hits or MCTrack)
include/fluxval/Plot.h     overlay + ratio panels, nulls, pull summary
